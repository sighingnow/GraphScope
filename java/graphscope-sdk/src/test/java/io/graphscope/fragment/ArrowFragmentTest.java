...
public class ArrowFragmentTest {
    ...
    public void cppLoadFragment() {
        loadConf();
        // String concat = "/home/admin/v6d/build/bin/arrow_fragment_test /tmp/vineyard.sock 2
        // \"/home/admin/v6d/gstest/modern_graph/knows.csv#header_row=true&delimiter=|&label=knows&src_label=person&dst_label=person\"
        // \"/home/admin/v6d/gstest/modern_graph/created.csv#header_row=true&delimiter=|&label=created&src_label=person&dst_label=software\"
        // 2 \"/home/admin/v6d/gstest/modern_graph/person.csv#label=person&header_row=true&delimiter=|\"
        // \"/home/admin/v6d/gstest/modern_graph/software.csv#label=software&header_row=true&delimiter=|\"";
    ...
    public void testFFIFragment() {
        ...
        for (int i = 0; i < fragment.vertexLabelNum(); ++i) {
            System.out
                    .print("vertex property num for vertex label [" + i + "] " + fragment.vertexPropertyNum(i) + ", ");
        }
        System.out.println();
        ...
        System.out.println(
                "inner vertices(" + innerVertices.begin().GetValue() + ", " + innerVertices.end().GetValue() + ")");
        System.out.println(
                "outer vertices(" + outerVertices.begin().GetValue() + ", " + outerVertices.end().GetValue() + ")");
        System.out.println(
                "frag vertices(" + fragVertices.begin().GetValue() + ", " + fragVertices.end().GetValue() + ")");
        ...
        System.out.println("outgoing adjlist for vertex " + cur.GetValue() + " oid " + fragment.getOid(cur) + " ["
                + outAdjList.begin().neighbor().GetValue() + " , " + outAdjList.end().neighbor().GetValue() + "] "
                + outAdjList.size());
        ...
        System.out.println("incoming adjlist for vertex " + cur.GetValue() + " oid " + fragment.getOid(cur) + " ["
                + inAdjList.begin().neighbor().GetValue() + " , " + inAdjList.end().neighbor().GetValue() + "]"
                + inAdjList.size());
        ...
        for (PropertyNbrUnit<Long> nbr : outRawAdjList.iterator()) {
            System.out.println(
                    "edge data " + nbr.getNeighbor().GetValue() + "/ " + nbr.vid() + ": " + columnDouble.get(nbr));
        }
        // the first property is string ,second is uint64_t
        VertexDataColumn<Long> vertexDataColumn = fragment.vertexDataColumn(0, 1, 2L);
        for (PropertyNbrUnit<Long> nbr : outRawAdjList.iterator()) {
            System.out.println("vertex data " + nbr.getNeighbor().GetValue() + "/ " + nbr.vid() + ": "
                    + vertexDataColumn.get(nbr.getNeighbor()));
        }
        ...
        System.out.println("vertex out degree " + fragment.getLocalOutDegree(cur, 0) + " in degree "
                + fragment.getLocalInDegree(cur, 0));
