...
public class GraphScopeClassLoader {
     ...
     */
    public static Object loadAndCreateObject(URLClassLoader classLoader, String className) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
    ...
    private static URL[] classPath2URLArray(String classPath) {
        ...
        Arrays.stream(splited).map(File::new).map(file -> {
            try {
                return file.toURL();
        ...
        System.out.println(String.join(":", Arrays.stream(res).map(URL::toString).collect(Collectors.toList())));
        return res;
    }
}
