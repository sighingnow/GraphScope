...
public class TraverseGraph500MainClass {
    public static void main(String[] args) {
        for (int i = 0; i < args.length; ++i) {
            System.out.println(i + " " + args[i]);
        }
        if (args.length != 2) {
            System.out.println("Usage: <vfile> <efile> ");
            System.exit(-1);
        }
        try {
            ...
            job.setMessageTypes(Long.class, LongMsg.class);
            ...
            configProperties.setProperty("messageTypes",
                    String.join(",", "gs::DoubleMsg=" + DoubleMsg.class.getName(),
                            "gs::LongMsg=" + LongMsg.class.getName()));
