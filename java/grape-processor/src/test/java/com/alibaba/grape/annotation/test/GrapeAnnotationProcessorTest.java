...
public class GrapeAnnotationProcessorTest {
    static void assertEquals(String str1, String str2) {
        if (str1.length() != str2.length()) {
            throw new IllegalArgumentException("Expected length " + str1.length() + ", got " + str2.length());
        }
        for (int i = 0; i < str1.length(); i++) {
            if (str1.charAt(i) != str2.charAt(i)) {
                throw new IllegalArgumentException(
                        "Expected char at " + i + " = " + str1.charAt(i) + ", got " + str2.charAt(i) + ", compared = "
                                + str1.substring(0, i + 1) + " vs. " + str2.substring(0, i + 1));
            }
        }
    }
    ...
    public void testMirror() throws IOException {
        JavaFileObject file1 = JavaFileObjects.forSourceLines("test.MirrorSample", "package test;",
                "import com.alibaba.fastffi.*;", "@FFIGen(library = \"test\")", "@FFIMirror",
                "@FFITypeAlias(\"MirrorSample\")", "@FFINameSpace(\"test\")",
                "public interface MirrorSample extends FFIPointer {", "  @FFIGetter int intField();",
                "  @FFISetter void intField(int value);",
        ...
        JavaFileObject file2 = JavaFileObjects.forSourceLines("test.Graph", "package test;",
                "import com.alibaba.fastffi.*;", "import com.alibaba.grape.annotation.*;", "@GraphType(",
                "  oidType = Integer.class,", "  vidType = Long.class,", "  vdataType = MirrorSample.class,",
                "  edataType = FFIByteString.class", ")", "public class Graph {", "}");

        {
            Compilation compilation = javac().withProcessors(new com.alibaba.fastffi.annotation.AnnotationProcessor(),
                    new GrapeAnnotationProcessor()).compile(file1, file2);
            checkCompilation(compilation);
        }
        {
            Compilation compilation = javac().withProcessors(new GrapeAnnotationProcessor(),
                    new com.alibaba.fastffi.annotation.AnnotationProcessor()).compile(file2, file1);
            checkCompilation(compilation);
        }
    }
    ...
    void checkCompilation(Compilation compilation) throws IOException {
        ...
        {
            String content = compilation.generatedFile(StandardLocation.SOURCE_OUTPUT, "grape-gen.h").get()
        ...
        {
            String content = compilation.generatedFile(StandardLocation.SOURCE_OUTPUT, "user_type_info.h").get()
                    .getCharContent(true).toString();
    ...
    public void testMessageTypes() {
        ...
        System.setProperty("grape.messageTypes",
                String.join(",", "jlong=" + Long.class.getName(), "jdouble=" + Double.class.getName()));
