...
public class GrapeAppScanner {
    ...
    /**
     * TODO: assume the config is in Java properties format.
     *
     * @return
     */
...
//    }
    private void readJobConf() {
        ...
        job.readConfig();
        String oidType = job.getOidType();
        String vidType = job.getVidType();
        String vdataType = job.getVdataType();
        String edataType = job.getEdataType();
        String messageTypes = job.getMessageTypesString();
        graphConfig = new GraphConfig(oidType, vidType, vdataType, edataType, messageTypes, "", "", "", "", "");
    }
    ...
    private String scanAppAndGenerate() {
        ...
        if (!graphScope) {
            readJobConf();
            graphConfig.messageTypes = messageTypes;
        } else {
        ...
        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
            if (attrs.isRegularFile()) {
                if (file.getFileName().toString().endsWith(".class")) {
                    ...
                    try {
                        // Why we need to load the class into the JVM rather than check it using bytecode
                        // library such as asm? This is because we need to guarantee that all dependency is
                        // available during the code generation.
                        Class<?> clazz = Class.forName(className, false, appClassLoader);
                        if (!className.equals(clazz.getName())) {
                            throw new IllegalStateException("Error: expected " + className + ", got " + clazz);
                        }
                        ...
                        if (ffiMirror != null) { // a FFIMirror
                            ...
                            if (ffiSynthetic == null) {
                                ...
                                ffiMirrors.put(className, ffiNameSpace.value() + "::" + ffiTypeAlias.value());
                            }
                        }
                        ...
                        if (Main.ignoreError) {
                            if (Main.verbose) {
                                System.err.println("WARNING: incompatible class error " + className);
                            }
                        } else {
                            throw new IllegalStateException(e);
                        }
