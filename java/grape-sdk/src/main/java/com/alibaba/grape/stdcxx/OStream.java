...
public interface OStream extends FFIPointer {
    /**
     * The signature of ostream is `ostream& put(const char c)` we will generate a native method: public void put(byte
     * c) { nativePut(getAddress(), c); } native void nativePut(long address, byte c); In the generated JNI cxx file:
     * void Java_xxx_nativePut(JNIEnv*, jclass, jlong address, jbyte c) { std::ostream* obj =
     * reinterpret_cast<std::ostream*>(address); obj->put(c); }
     *
     * @param c
     */
    void put(byte c);
