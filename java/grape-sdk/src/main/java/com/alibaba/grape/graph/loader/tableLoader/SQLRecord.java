...
public class SQLRecord implements WritableRecord {
  ...
  public void set(Writable[] values) throws IOException {
    if (cols.length != values.length) {
      throw new IOException("Size unmatch: " + values.length + ", expect: " + cols.length);
    }
  ...
  public String toString() {
    return "SQLRecord [cols=" + Arrays.toString(cols) + ", fields=" + Arrays.toString(fields) + "]";
  }
  ...
  private void checkColIndex(int index) {
    if (index >= size() || index < 0) {
      throw new RuntimeException("ODPS-0730001: Column index out of bound - col index '" + index
