...
public class GraphxEdgeManagerImpl<VD, ED, MSG_T> extends
    ...
    public GraphxEdgeManagerImpl(GraphXConf conf, VertexIdManager<Long, Long> idManager, VertexDataManager<VD> vertexDataManager) {
        this.conf = conf;
     ...
     */
    @Override
    public void iterateOnEdges(long srcLid, EdgeContextImpl<VD, ED, MSG_T> context,
        ...
        for (GrapeEdge<Long, Long,ED> edge : edgeIterable){
            ...
            logger.info("Edge ctx: srcLid{}, srcOid {}, dstLid {}, dstOid {}, dstVdata {}, edge value{}, iter size {}", srcLid, context.srcId(), edge.dstLid, edge.dstOid, vertexDataManager.getVertexData(edge.dstLid), edge.value, iterator.length());
            while (iterator.hasNext()){
                Tuple2<Long,MSG_T> tuple2 = iterator.next();
                logger.info("cur tuple: {}", tuple2);
                logger.info("srclid {}(oid {}) send {} to {} when visiting edge ({},{})",srcLid, idManager.lid2Oid(srcLid), tuple2._2(), tuple2._1(), edge.dstOid, edge.value);
