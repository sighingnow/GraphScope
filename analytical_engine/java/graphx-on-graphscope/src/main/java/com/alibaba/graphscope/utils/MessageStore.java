...
public interface MessageStore<T> extends PrimitiveArray<T> {
    ...
    void flushMessages(FixedBitSet nextSet, DefaultMessageManager messageManager,BaseGraphXFragment<Long,Long,?,?> fragment, int [] fid2WorkerId)
        throws IOException;

    void digest(FFIByteVector vector,BaseGraphXFragment<Long,Long,?,?> fragment, FixedBitSet curSet);

    static <T> MessageStore<T> create(int len, int fnum, int numCores,Class<? extends T> clz, Function2<T,T,T> function2, FixedBitSet nextSet)
        throws IOException {
