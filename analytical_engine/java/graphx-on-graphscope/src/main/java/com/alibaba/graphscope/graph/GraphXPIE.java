...
public class GraphXPIE<VD, ED, MSG_T> {
    ...
    public void init(IFragment<Long, Long, VD, ED> fragment, DefaultMessageManager messageManager,
        ...
        logger.info("edata array size {}, edge num{}", edataArray.getLength(),
            graphXFragment.getEdgeNum());
        if (edataArray.getLength() != graphXFragment.getEdgeNum()) {
            throw new IllegalStateException(
                "not equal" + edataArray.getLength() + "," + graphXFragment.getEdgeNum());
        }
        logger.info("vdata array size {}, frag vnum{}", vdataArray.getLength(),
            graphXFragment.getVerticesNum());
        if (vdataArray.getLength() != graphXFragment.getVerticesNum()) {
            throw new IllegalStateException(
                "not equal" + vdataArray.getLength() + "," + graphXFragment.getVerticesNum());
        }
    ...
    public void PEval() {
        ...
        for (long lid = 0; lid < innerVerticesNum; ++lid) {
            ...
            logger.info("Running vprog on {}, oid {}, original vd {}, cur vd {}", lid,
                graphXFragment.getId(vertex), originalVD, vdataArray.get(lid));
        }
    ...
    void flushOutMessage() {
        ...
        for (int i = bitSet.nextSetBit((int) innerVerticesNum); i >= 0;
            i = bitSet.nextSetBit(i + 1)) {
            v.SetValue((long) i);
            messageManager.syncStateOnOuterVertexGraphX(graphXFragment, v, vdataArray.get(i), vdataArray.get(i));
            logger.info("Frag {} send msg {} to outer v {}", graphXFragment.fid(),
                vdataArray.get(i), v.GetValue());
        }
        bitSet.clear();
    }
        ...
        if (outerMsgReceived) {
            ...
            for (int i = bitSet.nextSetBit(0); i >= 0; i = bitSet.nextSetBit(i + 1)) {
                if (i >= innerVerticesNum) {
                    throw new IllegalStateException(
                        "Not possible to receive a msg send to outer vertex");
                }
                vertex.SetValue((long) i);
                Long oid = graphXFragment.getId(vertex);
                VD originalVD = vdataArray.get(i);
                logger.info("Running vprog on {}, oid {}, original vd {}, cur vd {}", i,
                    graphXFragment.getId(vertex), originalVD, vdataArray.get(i));
            }
            bitSet.clear();
            for (long lid = 0; lid < innerVerticesNum; ++lid) {
                vertex.SetValue(lid);
        ...
        if (conf.getMsgClass().equals(Double.class) || conf.getMsgClass()
            ...
            while (messageManager.getMessageGraphX(graphXFragment, receiveVertex, msg, 2.0)) {
                if (receiveVertex.GetValue() >= verticesNum) {
                    throw new IllegalStateException(
                        "Receive illegal vertex " + receiveVertex.GetValue() + " msg: "
                            + msg.getData());
                }
