 ...
 */
@Private
class ThreadSafeOpenHashSet[@specialized(Long, Int, Double, Float) T: ClassTag](
                                                                       initialCapacity: Int,
                                                                       loadFactor: Double)
  extends Serializable {

  require(initialCapacity <= ThreadSafeOpenHashSet.MAX_CAPACITY,
    s"Can't make capacity bigger than ${ThreadSafeOpenHashSet.MAX_CAPACITY} elements")
  ...
  def union(other: ThreadSafeOpenHashSet[T]): ThreadSafeOpenHashSet[T] = {
  ...
  def union(other: OpenHashSet[T]): ThreadSafeOpenHashSet[T] = {
    val iterator = other.iterator
    while (iterator.hasNext) {
      add(iterator.next())
    }
   ...
   *                 to a new position (in the new data array).
   */
  private def rehash(k: T, allocateFunc: (Int) => Unit, moveFunc: (Int, Int) => Unit): Unit = synchronized{
    val newCapacity = _capacity * 2
    require(newCapacity > 0 && newCapacity <= ThreadSafeOpenHashSet.MAX_CAPACITY,
      s"Can't contain more than ${(loadFactor * ThreadSafeOpenHashSet.MAX_CAPACITY).toInt} elements")
