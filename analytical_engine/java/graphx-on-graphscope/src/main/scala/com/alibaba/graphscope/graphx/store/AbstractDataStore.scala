...
object AbstractDataStore extends Logging {
  val map = new ConcurrentHashMap[AbstractDataStore[_],DataStore[_]]()
  val count = new ConcurrentHashMap[AbstractDataStore[_],Int]()

  def set(key : AbstractDataStore[_], value : DataStore[_], numSplit : Int) :Unit = {
    require(!map.contains(key), s"map already contains key ${key}, numSplit ${numSplit}")
    map.put(key,value)
    count.put(key, numSplit)
  }
  def get(key : AbstractDataStore[_]) : DataStore[_] = {
