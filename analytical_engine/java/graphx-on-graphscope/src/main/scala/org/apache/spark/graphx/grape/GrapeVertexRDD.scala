...
//    val grapeVertexPartition = edgeRDD.grapePartitionsRDD.zipPartitions(edgeShuffle, preservesPartitioning = true){
    val grapeVertexPartition = PartitionAwareZippedBaseRDD.zipPartitions(SparkContext.getOrCreate(),edgeRDD.grapePartitionsRDD, edgeShuffle){
      (firstIter, edgeShuffleIter) => {
        ...
        val grapeVertexPartition = buildPartitionFromGraphX(ePart.pid, ePart.startLid, ePart.endLid, ePart.localId, ePart.localNum, ePart.siblingPid, ePart.client, ePart.graphStructure, edgeShuffleIter)
  ...
  def buildPartitionFromGraphX[VD: ClassTag](pid: Int, startLid : Long, endLid : Long,localId: Int, localNum : Int, sibllingPids : Array[Int], client: VineyardClient, graphStructure: GraphStructure, edgeShuffleIter: Iterator[(PartitionID,EdgeShuffle[VD,_])]):GrapeVertexPartition[VD] = {
...
//    val vertexDataView = new VertexDataStoreView[VD](vertexDataStore,startLid.toInt,endLid.toInt)
    ...
    new GrapeVertexPartition[VD](pid,startLid.toInt, endLid.toInt,localId,  localNum, sibllingPids, graphStructure, vertexDataStore, client, RoutingTable.fromGraphStructure(graphStructure))
  }
  ...
  def fromGrapeEdgeRDD[VD: ClassTag](edgeRDD: GrapeEdgeRDD[_], numPartitions : Int, defaultVal : VD, storageLevel: StorageLevel = StorageLevel.MEMORY_ONLY) : GrapeVertexRDDImpl[VD] = {
    ...
    val grapeVertexPartition = edgeRDD.grapePartitionsRDD.mapPartitions(iter =>{
      if (iter.hasNext){
        ...
        val grapeVertexPartition = GrapeVertexPartition.buildPrimitiveVertexPartition(defaultVal,ePart.pid, ePart.startLid, ePart.endLid, ePart.localId, ePart.localNum, ePart.siblingPid, ePart.client, ePart.graphStructure,RoutingTable.fromGraphStructure(ePart.graphStructure))
        Iterator(grapeVertexPartition)
      }
      else Iterator.empty
...
//        val vertexDataStore = new InHeapVertexDataStore[VD](array, ePart.client,0)
        val partition = new GrapeVertexPartition[VD](ePart.pid, 0,frag.getInnerVerticesNum.toInt, ePart.localId, ePart.localNum, ePart.siblingPid, actualStructure, vertexDataStore, ePart.client, RoutingTable.fromGraphStructure(actualStructure))
        Iterator(partition)
    }).cache()
