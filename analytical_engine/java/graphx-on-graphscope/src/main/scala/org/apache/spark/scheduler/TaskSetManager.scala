...
private[spark] class TaskSetManager(
  ...
  private[spark] def addPendingTask(
    ...
    for (loc <- tasks(index).preferredLocations) {
      log.info(s"adding pending task ${tasks(index)} with location ${loc.toString}")
      loc match {
        case e: ExecutorCacheTaskLocation =>
          pendingTaskSetToAddTo.forExecutor.getOrElseUpdate(e.executorId, new ArrayBuffer) += index
          log.info(s"adding task ${tasks(index)} to executor cache ${e.executorId}")
        case e: HDFSCacheTaskLocation =>
          log.info(s"adding to hdfs cache task${tasks(index)} ${e}")
          val exe = sched.getExecutorsAliveOnHost(loc.host)
          exe match {
            case Some(set) =>
              log.info(s"there are executors on ${set.mkString(",")}")
              for (e <- set) {
      ...
      log.info(s"adding task to host ${loc.host}, task index ${index}")
      pendingTaskSetToAddTo.forHost.getOrElseUpdate(loc.host, new ArrayBuffer) += index

      if (resolveRacks) {
        log.info(s"resolving for rack ${loc.host}")
    ...
    if (tasks(index).preferredLocations == Nil) {
      log.info(s"no pererred location is provided for ${tasks(index)}")
