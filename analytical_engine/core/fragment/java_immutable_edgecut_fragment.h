...
#ifndef ANALYTICAL_ENGINE_CORE_FRAGMENT_JAVA_IMMUTABLE_EDGECUT_FRAGMENT_H_
#define ANALYTICAL_ENGINE_CORE_FRAGMENT_JAVA_IMMUTABLE_EDGECUT_FRAGMENT_H_
...
class JavaImmutableEdgecutFragment
 ...
 public:
  ...
  JavaImmutableEdgecutFragment() = default;

  explicit JavaImmutableEdgecutFragment(std::shared_ptr<vertex_map_t> vm_ptr)
      : vm_ptr_(vm_ptr) {}

  virtual ~JavaImmutableEdgecutFragment() = default;
  ...
  void Serialize(const std::string& prefix) {
    ...
    snprintf(fbuf, sizeof(fbuf), kSerializationFilenameFormat, prefix.c_str(),
             fid_);
    VLOG(2) << "Persisting fragment to " << fbuf;
    auto io_adaptor =
  ...
  inline size_t GetOutgoingEdgeNum() const { return oenum_; }

  inline size_t GetIncomingEdgeNum() const { return ienum_; }
  ...
  inline VID_T GetGid(const OID_T& oid) const {
    OID_T internal_oid(oid);
    VID_T ret;
    if (vm_ptr_->GetGid(internal_oid, ret)) {
      return ret;
    } else {
  ...
  inline fid_t GetFragId(const vid_t& vid) {
    return IsInnerVertex(vid) ? fid_
                              : (fid_t)(ovgid_[vid - ivnum_] >> fid_offset_);
  }
  ...
  inline bool IsInnerVertex(const vid_t& vid) { return (vid < ivnum_); }

  inline bool IsOuterVertex(const vertex_t& v) const override {
    return (v.GetValue() < tvnum_ && v.GetValue() >= ivnum_);
  }
  inline bool IsOuterVertex(const vid_t& vid) {
    return (vid < tvnum_ && vid >= ivnum_);
  }
  ...
  inline VID_T GetOuterVertexGid(const vid_t& vid) {
    return ovgid_[vid - ivnum_];
  }
 ...
 public:
  ...
  inline adj_list_t GetIncomingAdjList(const vid_t& vid) {
    return adj_list_t(ieoffset_[vid], ieoffset_[vid + 1]);
  }
  inline nbr_t* GetIncomingAdjListBegin(const vertex_t& v) {
    return ieoffset_[v.GetValue()];
  }
  inline nbr_t* GetIncomingAdjListEnd(const vertex_t& v) {
    return ieoffset_[v.GetValue() + 1];
  }
  inline nbr_t* GetIncomingAdjListBegin(const vid_t& vid) {
    return ieoffset_[vid];
  }
  inline nbr_t* GetIncomingAdjListEnd(const vid_t& vid) {
    return ieoffset_[vid + 1];
  }
  ...
  inline adj_list_t GetOutgoingAdjList(const vid_t& vid) {
    return adj_list_t(oeoffset_[vid], oeoffset_[vid + 1]);
  }
  inline nbr_t* GetOutgoingAdjListBegin(const vertex_t& v) {
    return oeoffset_[v.GetValue()];
  }
  inline nbr_t* GetOutgoingAdjListEnd(const vertex_t& v) {
    return oeoffset_[v.GetValue() + 1];
  }
  inline nbr_t* GetOutgoingAdjListBegin(const vid_t& vid) {
    return oeoffset_[vid];
  }
  inline nbr_t* GetOutgoingAdjListEnd(const vid_t& vid) {
    return oeoffset_[vid + 1];
  }
  ...
  inline nbr_t* GetOutgoingInnerVertexAdjListBegin(const vertex_t& v) {
    assert(IsInnerVertex(v));
    assert(!oespliters_.empty());
    return oeoffset_[v.GetValue()];
  }
  inline nbr_t* GetOutgoingInnerVertexAdjListEnd(const vertex_t& v) {
    assert(IsInnerVertex(v));
    assert(!oespliters_.empty());
    return oespliters_[0][v.GetValue()];
  }
  /**
   * @brief Returns the outgoing adjacent inner vertices of v.
   *
