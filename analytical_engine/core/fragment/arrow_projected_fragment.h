...
class ArrowProjectedFragmentGroup
 ...
 public:
  ...
  std::string GetHostIdsStr() {
    std::stringstream ss;
    const char* separator = "";
    for (auto kv : fragments_) {
      auto objectId = kv.second;
      auto hostName = fragment_locations_[kv.first];
      ss << separator << hostName << ":" << objectId;
      separator = ",";
    }
    return ss.str();
  }
...
inline boost::leaf::result<vineyard::ObjectID> ConstructProjectedFragmentGroup(
  ...
  if (comm_spec.worker_id() == 0) {
    ...
    for (fid_t i = 0; i < comm_spec.fnum(); ++i) {
      builder.AddFragmentObject(i,
                                gathered_object_ids[comm_spec.FragToWorker(i)],
    ...
    MPI_Gather(&my_len, 1, MPI_INT, NULL, 1, MPI_INT, grape::kCoordinatorRank,
               comm_spec.comm());
    MPI_Gatherv(my_host_name.c_str(), my_len, MPI_CHAR, NULL, NULL, NULL,
